...
def start_dispersy():
    # Start up the API and load the user
    api = MarketAPI(MockDatabase(PersistentBackend('.')))
    try:
        priv = api.db.backend.get_option('user_key_priv')
        user = api.login_user(priv.encode("HEX"))
    except IndexError:
        user, pub, priv = api.create_user()
    ...
    my_member = dispersy.get_member(private_key=priv.decode("HEX"))
    ...
    LoopingCall(lambda: community.send_model_request([('users',
                                                       ''),
