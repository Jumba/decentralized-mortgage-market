...
class MultiChainDB(Database):
    ...
    def get_latest_hash(self, public_key):
        ...
        db_query = u"SELECT hash_block FROM (" \
                   u"SELECT hash_block, sequence_number_benefactor AS sequence_number " \
                   u"FROM multi_chain WHERE sequence_number_benefactor = (" \
                   u"SELECT MAX(sequence_number_benefactor) FROM multi_chain WHERE benefactor = ?)" \
                   u"UNION " \
                   u"SELECT hash_block, sequence_number_beneficiary AS sequence_number " \
                   u"FROM multi_chain WHERE sequence_number_beneficiary = (" \
                   u"SELECT MAX(sequence_number_beneficiary) FROM multi_chain WHERE beneficiary = ?))"
